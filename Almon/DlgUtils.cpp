...
ExporterBase::Format ExporterBase::GetFormatFromFile(const CString& file) // Unknown extensions return 'Format::MAX'
{
...
CString DlgUtils::SelectExportFile(CWnd* pParent, ExporterBase* pExporter, CString defaultFileName)
{
	...
	auto AddExtension = [&](const CString& type, const CString& ext)
	{
		if (!filter.IsEmpty())
			filter.Append("|");
		filter.AppendFormat(_T("%s|*%s"), type.GetString(), ext.GetString());
	};	
