...
void DlgFilter::EnableTreeViewMode()
{
	...
	CRect rcBtn = GetControlWindowRect(IDOK);
	int height = rcBtn.Height();
	rcBtn.bottom = rcClient.bottom - 10;
	rcBtn.top = rcBtn.bottom - height;
	MoveControl(IDOK, rcBtn);

	rcBtn = GetControlWindowRect(IDCANCEL);
	rcBtn.bottom = rcClient.bottom - 10;
	rcBtn.top = rcBtn.bottom - height;
	MoveControl(IDCANCEL, rcBtn);
}
...
void DlgFilter::UpdateControls()
{
	auto UpdateCheck = [&](int idChkMin, int idEdMin, int idChkMax, int idEdMax)
		{
			...
			EnableControl(idEdMin, bMinChecked);
			EnableControl(idChkMax, bMinChecked);
			EnableControl(idEdMax, bMinChecked && IsButtonChecked(idChkMax));
		};
	...
	EnableControl(IDC_EDIT_INCLUDE_TEXT, IsButtonChecked(IDC_CHECK_INCLUDE_TEXT));
	EnableControl(IDC_EDIT_EXCLUDE_TEXT, IsButtonChecked(IDC_CHECK_EXCLUDE_TEXT));
}

void DlgFilter::OnComboSelChange()
{
	if (auto pComboBox = GetCurrentControl<CComboBox>(); pComboBox && pComboBox->GetCurSel() == 0)
		SetControlFocus(IDC_BUTTON_SHOW_FILTER);
}
...
void DlgFilter::InitDefaultValues()
{
	...
	FillRange(rangeNumAllocs, IDC_CHECK_ALLOCS_MIN, IDC_EDIT_ALLOCS_MIN, IDC_CHECK_ALLOCS_MAX, IDC_COMBO_ALLOCS_MAX);
