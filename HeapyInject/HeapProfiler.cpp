...
void __declspec(dllexport) GetSymbol(UINT_PTR addr, char* symbol, long& symbolLen)
{
...
void HeapProfiler::AddressTranslatorThread()
{
	...
	while (!m_bExitThreads)
	{
		...
		if (waitResult == WAIT_OBJECT_0)
		{
			// Get the address
			UINT_PTR address{};
			CopyMemory((PVOID)&address, (PVOID)(m_memFileAddr.GetBuffer()), sizeof(UINT_PTR));
...
{
	AllocInfo allocInfo;
	allocInfo.numBytes = size;
	allocInfo.addr = (UINT_PTR)ptr;
	allocInfo.numFrames = CaptureStackBackTrace(FRAMES_SKIP, BACKTRACE_SIZE, (PVOID*)(allocInfo.framesAddr), 0);

	AcquireSRWLockExclusive(&m_lock);
	if (m_pWrite + sizeof(AllocInfo) > m_pBufferLast) // Overflow
	{
	...
	CopyMemory(m_pWrite, &allocInfo, sizeof(AllocInfo));
	m_pWrite += sizeof(AllocInfo);
...
{
	FreeInfo freeInfo;
	freeInfo.addr = (UINT_PTR)ptr | FREE_MASK;

	AcquireSRWLockExclusive(&m_lock);
	if (m_pWrite + sizeof(FreeInfo) > m_pBufferLast) // Overflow
	{
		m_pLapLast = m_pWrite; // Remember the last write position for this lap
		m_pWrite = m_pBufferFirst;
		m_writeLap++;
	}
	CopyMemory(m_pWrite, &freeInfo, sizeof(FreeInfo));
	m_pWrite += sizeof(FreeInfo);
	ReleaseSRWLockExclusive(&m_lock);
}
